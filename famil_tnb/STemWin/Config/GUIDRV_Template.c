...
uint32_t UCGUI_LCD_CMD  = 0x00007FFFE;
uint32_t UCGUI_LCD_DATA = 0x00007FFFF; 
/*********************************************************************
*
*       Defines
*
...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
  LCD_Fast_DrawPoint(x,y,PixelIndex);	//快速画点
}
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  LCD_Fill(x0,y0,x1,y1,LCD_COLORINDEX);	//填充单色
}
 ...
 *(__IO uint16_t*)(UCGUI_LCD_CMD) = lcddev.wramcmd;
 ...
 {
   pixel = *p;
   *(__IO uint16_t*)(UCGUI_LCD_CMD) = pixel;
 }
}
